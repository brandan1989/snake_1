...
function snakeSquadLoop() {
  ...
  /*Logic of the game. from start to finish*/
  calculateSnakeMove();
  drawSnake();
  drawApple();
  if (snakeTouchesApple() == true) {
    lengthenSnakeByOne();
...
function drawSnake(){
  ...
  for (let i = 0; i < snake.cells.length; i++) {
    ...
    if (i === 0){ //if the current cell is in the head, the avatar will draw
      drawCellWithBitmoji(current_cell);
    } else {
      // draws 1 px smaller than the grid
      context.fillStyle = snake.color;
      context.fillRect(current_cell.x, current_cell.y, grid-1, grid-1);
    }
  }
}
...
function checkCrashItself(){
  /* TO DO */
    for (let i = 0; i < snake.cells.length; i++) {
    let currect_cell = snake.cells[i];
    for (let j = i + 1; j < snake.cells.length; j++){
      let other_cell = snake.cells[j];
      //if the cell occupies same space as the other cell, the game will reset
      if (currect_cell.x === other_cell.x && currect_cell.y === other_cell.y) {
